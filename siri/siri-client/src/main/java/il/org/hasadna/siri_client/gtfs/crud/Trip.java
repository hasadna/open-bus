...
public interface Trip {
	...
	/**
	 * (Optional) The trip_headsign field contains the text that appears on a
	 * sign that identifies the trip's destination to passengers. Use this field
	 * to distinguish between different patterns of service in the same route.
	 * If the headsign changes during a trip, you can override the trip_headsign
	 * by specifying values for the stop_headsign field in stop_times.txt.
	 * 
	...
	/**
	 * (optional) The direction_id field contains a binary value that indicates
	 * the direction of travel for a trip. Use this field to distinguish between
	 ...
	 * time tables. You can specify names for each direction with the
	 * trip_headsign field.
	 * 
	 * @return Direction Id
	 */
