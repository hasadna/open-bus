...
public interface StopTime {
	 ...
	 * the beginning of the service day. For times occurring after midnight on the
	 ...
	 * and departure times are timepoints. For example, if a transit vehicle arrives
	 * at a stop before the scheduled departure time, it will hold until the
	 * departure time. If this stop is not a timepoint, use either an empty string
	 * value for the arrival_time field or provide an interpolated time. Further,
	 * indicate that interpolated times are provided via the timepoint field with a
	 * value of zero. If interpolated times are indicated with timepoint=0, then
	 * time points must be indicated with a value of 1 for the timepoint field.
	 * Provide arrival times for all stops that are time points.
	 * <p>
	 ...
	 * occur) at the beginning of the service day. For times occurring after
	 * midnight on the service day, enter the time as a value greater than 24:00:00
	 ...
	 * at a stop before the scheduled departure time, it will hold until the
	 * departure time. If this stop is not a timepoint, use either an empty string
	 * value for the departure_time field or provide an interpolated time (further,
	 * indicate that interpolated times are provided via the timepoint field with a
	 * value of zero). If interpolated times are indicated with timepoint=0, then
	 * time points must be indicated with a value of 1 for the timepoint field.
	 * Provide departure times for all stops that are time points.
	 * <p>
	 * A departure time must be specified for the first and the last stop in a trip
	 * even if the vehicle does not allow boarding at the last stop. Times must be
	 * eight digits in HH:MM:SS format (H:MM:SS is also accepted, if the hour begins
	 * with 0). Do not pad times with spaces. The following columns list stop times
	 * for a trip and the proper way to express those times in the departure_time
	 * field:
	 * 
	 ...
	 * stop_times.txt must have location_type of 0. Where possible, stop_id values
	 * should remain consistent between feed updates. In other words, stop A with
	 * stop_id 1 should have stop_id 1 in all subsequent data updates. If a stop is
	 * not a time point, enter blank values for arrival_time and departure_time.
	 * 
	 * @return Stop Id
	 */
	 ...
	 * must increase along the trip. For example, the first stop on the trip could
	 * have a stop_sequence of 1, the second stop on the trip could have a
	 * stop_sequence of 23, the third stop could have a stop_sequence of 40, and so
	 * on.
	 * 
